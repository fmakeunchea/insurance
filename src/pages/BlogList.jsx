    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="max-w-5xl mx-auto px-4 py-14">
        <div className="grid sm:grid-cols-2 gap-6">
          {blogPosts.map((p) => (
            <Link key={p.slug} to={`/blog/${p.slug}`} className="group bg-white rounded-2xl border border-navy-100 hover:border-gold-200 p-7 hover:shadow-lg hover:-translate-y-1 transition-all flex flex-col">
      ...
      <section className="bg-navy-900 py-16">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/#quick-form" className="inline-flex items-center justify-center gap-2 px-7 py-4 bg-gold-500 hover:bg-gold-600 text-white font-semibold rounded-xl"><Calendar className="w-5 h-5" />Get Free Quote</Link>
            <a href="https://calendly.com/harnordinc" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 px-7 py-4 bg-white/10 border border-white/20 text-white font-semibold rounded-xl">Book Consultation</a>
          </div>
        </div>
      </section>
    </div>
